...
////////////////////
		...
		var loopFiles = {
			...
			cssMaster: ['src/scss/styles.scss'],
...
function compileJS(){
	var sectionDir = 'src/components/',
			components;
	//Run through components and boot the app
...
function cssBoot(){
	...
	var booter = loopFiles.cssMaster,
		newFileName = 'dbcss'
...
gulp.task('theme:upload', function() {
	gulp.src(watchSrc)
		.pipe(watch(watchSrc))
		.pipe(filterChanged)
		.pipe(shell([
...
gulp.task('watch',['build'], function(){
	bsyncInit(serverName);

	//Watch Scss
	gulp.watch([loopFiles.cssSrc, adminFiles.cssSrc], function(response){
		var changeLocation = changeChecker('scss', response);
		console.log('Change in ' + changeLocation.section + ':' + changeLocation.page);

		if(changeLocation.section === 'admin'){
			compileCSS('admin');
		}
		if(changeLocation.section === 'loop'){
			compileCSS('loop');
		}
	});

	//Watch Vue Components
	gulp.watch([loopFiles.vueSrc, adminFiles.vueSrc], function(response){
...
gulp.task('default', ['theme:upload', 'theme:delete'], function() {
	//Watch Scss
	gulp.watch([loopFiles.cssSrc], ['compileSCSS']);
	//Watch Vue Components
	gulp.watch([loopFiles.vueSrc], function(response){
		var changeLocation = changeChecker('component', response);
		console.log('Change in ' + changeLocation.section + ':' + changeLocation.page);
	...
	//Watch Core JS
	gulp.watch([loopFiles.jsSrc], ['compileJS']);
});
